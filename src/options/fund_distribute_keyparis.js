...
export async function fund_distribute_keypairs() {
  try {
    ...
    //  -----------------------------------------------------------------------------------------
    //  Distribute SOL to distributed Wallets
    //  -----------------------------------------------------------------------------------------
    const distributeKeypairsFile = fs.readFileSync("data/distributeKeypairs.json", "utf8");
    const distributeKeypairs = JSON.parse(distributeKeypairsFile);

    for (const [index, kp] of distributeKeypairs.entries()) {
